...
const CitizenSidebar = ({
  ...
  const mainMenuItems = [
    ...
    { id: 'Monthly Payments', label: 'Monthly Payments', icon: CreditCard, badge: '₹' },
    { id: 'Collection Records', label: 'Collection Records', icon: FileText },
    { id: 'AI Assistant', label: 'EcoMind AI Chat', icon: Bot },
  ];

  const bottomMenuItems = [
    { id: 'Settings', label: 'Theme & Settings', icon: Sliders },
    { id: 'Help & Guidelines', label: 'Help & Guidelines', icon: HelpCircle },
  ];
  ...
  const isItemActive = (item) => {
    ...
    if (item.id === 'AI Assistant' && (activeItem === 'EcoMind AI Chat' || activeItem === 'AI Assistant')) return true;
    if (item.id === 'Settings' && (activeItem === 'Theme & Settings' || activeItem === 'Settings')) return true;
    if (item.id === 'Help & Guidelines' && (activeItem === 'Help & Guidelines' || activeItem === 'Guidelines')) return true;
    return false;
  };
  ...
  return (
    <>
      ...
      >
        {/* Top Fixed Section: Collapse Button & User Profile */}
        <div className="shrink-0 px-3 pt-2.5 pb-2 space-y-2 border-b border-gray-100/60 bg-white">
          ...
          <div
            ...
            className={`w-full bg-[#f4f9f5] hover:bg-[#ebf6ed] border border-emerald-100/70 rounded-xl ${
              ...
              activeItem === 'Profile' ? 'ring-2 ring-[#0a4d2c]/30 bg-[#ebf6ed]' : ''
            }`}
            title="Click to view & edit Profile"
          >
            <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-2.5 min-w-0'}`}>
              <div className="relative shrink-0">
                <div className="w-9 h-9 rounded-full bg-[#0a4d2c] text-white flex items-center justify-center font-bold text-sm shadow-xs group-hover:scale-105 transition-transform">
              ...
              {!isCollapsed && (
                <div className="min-w-0 text-left">
                  <p className="text-[13px] font-bold text-gray-900 truncate leading-tight group-hover:text-[#0a4d2c] transition-colors">
                    {userName}
                  </p>
            ...
            {!isCollapsed && (
              <ChevronRight className="w-3.5 h-3.5 text-gray-400 group-hover:text-gray-700 transition-colors shrink-0 ml-1" />
            )}
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto custom-scrollbar px-2.5 py-2 space-y-1">
          {mainMenuItems.map((item) => {
            ...
            return (
              <div key={item.id} className="relative group">
                ...
                >
                  {/* Leaf watermark for active item */}
                  {isActive && (
                  ...
                  <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-2.5 min-w-0'} relative z-10`}>
                    {isActive ? (
                    ...
                    {!isCollapsed && (
                      <span
                        className={`text-[12.5px] font-semibold truncate ${
                          isActive ? 'text-white font-bold' : 'text-gray-800 group-hover:text-emerald-950'
                        }`}
                      >
                        {item.label}
                      </span>
          ...
          {/* Subtle Divider Line */}
          <div className="py-1">
            <hr className="border-t border-gray-100 mx-1" />
          </div>
        ...
        <div className="shrink-0 px-3 pt-2 pb-2.5 space-y-2 border-t border-emerald-50 bg-white">
          {/* Compact Scenic Card: "Together for a Cleaner Tomorrow" */}
          {!isCollapsed && (
